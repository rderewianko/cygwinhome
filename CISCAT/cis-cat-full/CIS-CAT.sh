...
echo $JAVA_VERSION_RAW | grep -i 'java version' | grep '[1]\.[567]\.[0-9]' 2>&1 > /dev/null
...
if [ $? -eq "1" ]; then
        ...
        echo ""
        echo "You must use Java 1.5.0_22, 1.6.x, or 1.7.x. The most recent version of Java is recommended."        
        exit 1;
fi
